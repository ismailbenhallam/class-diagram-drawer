...
public class ReflectionUtilities {
    ...
    /*
     * Interdire l'instanciation de la class, car elle ne contient que les mèthodes
     * static
     */
...
//			return declaration.toString().replace('$', '.');
    ...
    public static String getClassName(Class<?> clazz) {
        if (clazz.getTypeParameters().length > 0) {
            StringBuilder buffer = new StringBuilder(clazz.getSimpleName());
            buffer.append("<");
    ...
    public static String getMethodInfo(Method m) {
        ...
        methodInfo.append(getVisibility(m)).append(m.getName()).append("(");
        ...
        for (int i = 0; i < p.length; i++) {
            ...
            methodInfo.append(p[i].getName()).append(": ");
            methodInfo.append(ReflectionUtilities.getDeclaration(p[i].getParameterizedType()));
        }
        methodInfo.append(") : ").append(getDeclaration(m.getGenericReturnType())).append(System.lineSeparator());
    ...
    public static String getFieldInfo(Field f) {
        ...
        if (ReflectionUtilities.isFinal(f) && ReflectionUtilities.isStatic(f)) {
            try {
                ...
                fieldInfo.append(f.getName().toUpperCase()).append(" : ").append(ReflectionUtilities.getDeclaration(f.getGenericType())).append(" = \"").append(f.get(null).getClass().isArray() ? Arrays.toString((Object[]) f.get(null))
                    : f.get(null)).append("\"");
    ...
    public static Map<Integer, Set<Class<?>>> getClassesByInheritanceLevel(Set<Class<?>> allClasses) {
         ...
         */
        for (Class<?> clazz : allClasses.toArray(new Class<?>[0])) {

            if (
                // Si c'est une annotation ou enum
                clazz.isAnnotation() || clazz.isEnum()
                    // Si c'est une classe qui n'hèrite pas d'une classes interne (pas dans le Set
                    // de classes passè en param), et qu'elle n'implemente pas d'interfaces internes
                    || clazz.getSuperclass() != null
                    && !allClasses.contains(clazz.getSuperclass())
                    && !containsOne(allClasses, clazz.getInterfaces())

                    // Si c'est une interface qui n'hèrite d'aucune interface, ou qu'elle
                    // n'hèrite pas d'interfaces internes
                    || clazz.isInterface()
    ...
    /*
     * 'classes' contient les Classes qu'on a pas encore dèfinit leurs niveaux, et
     * 'supLevelClasses' contient les classes du niveau N-1 , cette fonction
     * dètèrmine les classes du niveau N
     */
    private static Map<Integer, Set<Class<?>>> getClassesLevels(Integer level, Set<Class<?>> classes,
        ...
        for (Class<?> clazz : classes) {
            ...
            // si cette classe/interface implemente/hèrite d'une interface du niveau N-1
